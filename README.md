...
| `batch-process-freq` | uint32 | 1 – 2³²−1 | How often (in seconds) a new process is auto-generated by `scheduler-start` / `scheduler-test` |
...
- Setting `batch-process-freq 1` generates a new process every second; use a higher value (e.g. `10`) to slow down batch generation.
...
Starts continuous automatic process generation. Every `batch-process-freq` seconds, a new process named `p01`, `p02`, `p03`, ... is created and enqueued.
...
Scheduler execution timing — `quantum-cycles`, `delay-per-exec`, and `SLEEP` — is measured in these ticks. Batch process generation uses `batch-process-freq` as wall-clock seconds so recorded wait times map predictably to generated process counts.
